...
class ScriptedTransport implements QboTransport {
  ...
  async request(req: QboRequest): Promise<QboResponse> {
    ...
    return {
      status: 200,
      headers: {},
      body: { QueryResponse: { [entity]: rows, startPosition: 1, maxResults: rows.length } },
    };
  }
}
...
async function seedProduct(fields: {
  ...
  const { data } = await admin
    ...
    .insert({
      ...
      external_ids: {
        qbo: '101',
        qbo_fp: productFingerprint({ name: fields.storedName, status: 'active' }),
      },
...
const run = (rows: Partial<Record<string, unknown[]>>, runId: string) =>
  syncIncremental(
    admin,
    adapterFor(rows),
    { tenantId, connectionId, syncRunId: runId },
...
describe('syncIncremental — conflict policy', () => {
  ...
  it('LWW: local edited + remote newer → applies remote, logs accept_remote', async () => {
    ...
    await run(
      {
        Item: [
          item({ Name: 'QBO Bolt', MetaData: { LastUpdatedTime: '2026-06-09T00:00:00-00:00' } }),
        ],
      },
      await newRun(),
    );
    ...
    expect(c[0]).toEqual({
      policy_decision: 'last_write_wins',
      applied_resolution: 'accept_remote',
    });
  });
      ...
      Line: [
        {
          ...
          ItemBasedExpenseLineDetail: { ItemRef: { value: '101' }, Qty: 50 },
        },
      ],
