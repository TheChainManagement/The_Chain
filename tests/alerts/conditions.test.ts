...
describe('buildFireableAlerts', () => {
  it('fires reorder_due from an open recommendation with urgency-based severity', () => {
    const out = buildFireableAlerts({
      ...EMPTY,
      openRecs: [
        { productId: 'p1', sku: 'BLT-200', name: 'Bolt', recommendedQty: 47, position: -3, reorderPoint: 20, safetyStock: 10, daysOfSupply: 0 },
        { productId: 'p2', sku: 'WSH-1', name: 'Washer', recommendedQty: 10, position: 5, reorderPoint: 20, safetyStock: 8, daysOfSupply: 4 },
        { productId: 'p3', sku: 'NUT-9', name: 'Nut', recommendedQty: 5, position: 18, reorderPoint: 20, safetyStock: 8, daysOfSupply: 12 },
      ],
    });
    expect(out).toHaveLength(3);
    expect(out.find((a) => a.entityId === 'p1')).toMatchObject({ kind: 'reorder_due', severity: 'critical' }); // out of stock
  ...
  it('a SKU with an open recommendation does NOT also fire stockout/overstock', () => {
    const out = buildFireableAlerts({
      ...EMPTY,
      openRecs: [{ productId: 'p1', sku: 'BLT', name: 'Bolt', recommendedQty: 5, position: 2, reorderPoint: 20, safetyStock: 10, daysOfSupply: 3 }],
      // Same SKU also looks high-risk + (absurdly) overstocked in policy — suppressed.
      policies: [{ productId: 'p1', sku: 'BLT', name: 'Bolt', daysOfSupply: 3, stockoutRiskScore: 0.9, reorderPoint: 20 }],
    });
  ...
  it('stockout_risk thresholds: <0.25 silent, then info/warn/critical', () => {
    const policy = (score: number) => ({ productId: `p${score}`, sku: 'S', name: 'S', daysOfSupply: 5, stockoutRiskScore: score, reorderPoint: 10 });
    const out = buildFireableAlerts({ ...EMPTY, policies: [policy(0.1), policy(0.3), policy(0.5), policy(0.8)] });
  ...
  it('overstock fires only above the DOS floor and never alongside stockout risk', () => {
    const out = buildFireableAlerts({
      ...EMPTY,
      policies: [
        { productId: 'a', sku: 'A', name: 'A', daysOfSupply: 200, stockoutRiskScore: 0, reorderPoint: 10 }, // warn overstock
        { productId: 'b', sku: 'B', name: 'B', daysOfSupply: 100, stockoutRiskScore: 0, reorderPoint: 10 }, // info overstock
        { productId: 'c', sku: 'C', name: 'C', daysOfSupply: 40, stockoutRiskScore: 0, reorderPoint: 10 }, // nothing
      ],
    });
