...
- `locations` (tenant_id, id, name, type enum: warehouse/store/plant/third_party/consignment, address jsonb, active, created_at). Multi-location wired from Wave 1; Wave 1 UI shows a single auto-created "Main" location per tenant.
...
- `stock_movements` (id, tenant_id, product_id, location_id, type enum: sale/receipt/transfer_in/transfer_out/adjustment/cycle_count, quantity numeric, source enum: qbo/csv/manual/api/workflow/rutter, source_ref text, occurred_at, created_at). Append-only ledger. **Partitioned by RANGE(occurred_at) yearly.** Indexes: (tenant_id, product_id, occurred_at), (tenant_id, location_id, occurred_at).
