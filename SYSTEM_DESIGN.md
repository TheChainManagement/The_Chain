...
- `inventory_levels` (tenant_id, product_id, location_id, on_hand numeric, **on_hold numeric not null default 0** (W2-2.5: a sub-bucket of on_hand — held goods stay owned and valued, excluded from available-to-promise), allocated numeric, in_transit numeric, **avg_unit_cost numeric(14,4)** (W2-2.5: moving-average cost, stock-UoM basis, maintained ONLY by the posting kernel on costed receipts), **avg_cost_provenance enum-check: seeded/posted**, last_counted_at, updated_at, primary key (tenant_id, product_id, location_id)). Indexed on (tenant_id, location_id) and (tenant_id, product_id). **Member write policies REMOVED in W2-2.5 — balances mutate only through the posting kernel (service-role path).** Position formula everywhere: `on_hand − on_hold + in_transit − allocated` via the single helper `src/lib/inventory/position.ts`.
- `stock_movements` (id, tenant_id, product_id, location_id, type enum: sale/receipt/transfer_in/transfer_out/adjustment/cycle_count/**issue_out/issue_return/return_to_vendor/customer_return** (W2-2)/**hold/release** (W2-2.5: positive qty; move the on_hold bucket, never on_hand), quantity numeric, source enum: qbo/csv/manual/api/workflow/rutter, source_ref text, **demand_ref_type text nullable** (work_order/crew/cost_center — the polymorphic consuming-object envelope), **demand_ref_id text nullable** (free-text ref, intentionally NOT a FK), **reason_code text nullable**, **note text nullable**, occurred_at, created_at). Append-only ledger. **Partitioned by RANGE(occurred_at) yearly.** Indexes: (tenant_id, product_id, occurred_at), (tenant_id, location_id, occurred_at), partial (tenant_id, demand_ref_type, demand_ref_id, occurred_at) where demand_ref_id is not null. CHECKs (W2-2): issue_out needs a demand ref + negative qty; issue_return the same ref + positive qty; return_to_vendor negative; customer_return positive.
- `inventory_op_events` (id uuid, tenant_id, kind enum-check: issue/adjustment/cycle_count_close/hold (W2-2.5), actor_user_id nullable, idempotency_key, summary jsonb, created_at, unique (tenant_id, idempotency_key)). W2-2 operator idempotency ledger: one row per APPLIED operator posting; a replayed key is a no-op. Written only by the posting RPCs (`post_issue_movements`, `post_stock_adjustment`, `close_cycle_count_session`) — the posting-kernel prototypes the W2-2.5 kernel formalizes. Member-select RLS; audit-triggered.
...
- `product_suppliers` (tenant_id, product_id, supplier_id, supplier_sku, unit_cost, lead_time_days int, moq int, is_primary, **purchase_uom text** (W2-2.5: the unit this supplier sells in), **purchase_to_stock_factor numeric(14,4) CHECK > 0** (1 purchase unit = factor stock units; null = same unit, factor 1), primary key (tenant_id, product_id, supplier_id)). When a factor is set, `unit_cost` and all PO-line quantities/costs for that supplier are in PURCHASE UoM; the ledger and `inventory_levels` stay in stock UoM only, always.
...
## Inventory posting kernel (W2-2.5)

**The anchor-point architecture decision:** every LEDGER-BACKED balance mutation (on_hand, on_hold, the moving average) flows through ONE SQL primitive — `post_stock_movement()`. The `in_transit` commitment is the one non-ledger balance: it moves only inside the two kernel-surface functions defined in the same migration (`apply_po_approval` commits it, a PO-sourced receipt drains it through the kernel's `affects_in_transit`). Future modules (W2-3 procurement, logistics, maintenance) touch stock ONLY through this kernel; a module writing `inventory_levels` or inserting `stock_movements` any other way is a defect.

**`post_stock_movement(tenant, product, location, type, qty, source, source_ref, occurred_at, demand_ref_type?, demand_ref_id?, reason_code?, note?, unit_cost?, affects_in_transit?)`** — SECURITY INVOKER, service-role caller, atomic under the level row lock:
- Validates the type contract (signs, required demand refs, required reasons) and writes the ledger row + the balance move in one transaction.
- **Moving-average cost** (Item 2b): on a costed `receipt` — `on_hand <= 0` or no prior average → `avg = receipt cost`; else `avg = ((on_hand × old_avg) + (qty × cost)) / (on_hand + qty)`; provenance flips to `posted`. Every other type leaves the average untouched and is valued at current average. Seeded averages (from the primary supplier link at migration time) carry provenance `seeded`.
- **on_hold bucket** (Item 2c): `hold` moves un-held shelf stock into `on_hold` (guard: cannot exceed `on_hand − on_hold`); `release` drains it (guard: cannot exceed `on_hold`). `on_hand` never moves — held stock stays owned and valued (MG 2026-07-09) and out of available-to-promise.
- **in_transit**: a PO-sourced receipt (`affects_in_transit`) drains the in-transit commitment `apply_po_approval` made, floored at 0.

**Callers own orchestration, never balances.** The domain RPCs wrap the kernel with their own idempotency ledgers and status transitions: `receive_purchase_order` (po_receipt_events), `post_issue_movements` / `post_stock_adjustment` / `close_cycle_count_session` / `post_stock_hold` (inventory_op_events), `onboarding_seed_first_product` (posts the opening balance as an `onboarding_seed` adjustment so the ledger explains a tenant's stock from its first minute).

**The ingestion door — `record_stock_movements(tenant, rows jsonb)`:** CSV import and QBO sync append HISTORICAL movements (demand history for the forecaster) through this set-based, balance-NEUTRAL entry — imported history is already reflected in imported on-hand; replaying it into balances would double-count. Idempotent on `(tenant, source, source_ref, occurred_at)`. TS façade: `src/lib/inventory/post-movement.ts`.

**UoM conversion (Item 2a):** PO lines order in PURCHASE UoM. `apply_po_approval` commits `in_transit` in stock units (`ordered × factor`); `receive_purchase_order` converts each line's received delta (`qty × factor` → ledger, `unit_cost ÷ factor` → the moving average); `convert_recommendations_to_po` divides stock-UoM recommendations by the factor (fractional allowed — MG 2026-07-09, the receive UI flags remainders). `supplier_performance` stays in purchase UoM: the unit the promise was made in.

**Enforcement:** member RLS write policies on `inventory_levels` (insert/update) and `stock_movements` (insert) are dropped; the service-role RPC path is the only door, and the app-layer role gate in each Server Action is the authorization. The acceptance test (`tests/inventory/kernel.test.ts`) proves ledger replay equals stored balances for a kernel-only product.

**Explicitly deferred (recorded, not lost):** FIFO cost layers, landed cost (freight/duty allocation), GL integration, three-way match.
